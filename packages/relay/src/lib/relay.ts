...
export class RelayImpl implements Relay {
  ...
  initClient(logger: Logger, hederaNetwork: string, type: string | null = null): Client {
    ...
    logger.info(`SDK client successfully configured to ${JSON.stringify(hederaNetwork)} for account ${client.operatorAccountId} with request timeout value: ${client.requestTimeout}`);
